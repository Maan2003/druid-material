...
use druid::{Affine, Data, Insets, LinearGradient, Point, TimerToken, UnitPoint, theme};

// the minimum padding added to a button.
// NOTE: these values are chosen to match the existing look of TextBox; these
// should be reevaluated at some point.
const LABEL_INSETS: Insets = Insets::uniform_xy(10., 10.);
...
impl<T: Data> Button<T> {
    ...
    pub fn from_label(label: Label<T>) -> Button<T> {
        Button {
            label: label.with_text_color(crate::color::ON_PRIMARY),
            label_size: Size::ZERO,
            expanding_time: None,
            expanding_radius: None,
            click_pos: Point::ZERO,
        }
    }
...
impl<T: Data> Widget<T> for Button<T> {
        ...
        match event {
            ...
            Event::MouseUp(m) => {
                if ctx.is_active() {
                    ...
                    let timer = ctx.request_timer(Duration::from_millis(400));
                    self.click_pos = m.pos;
                    self.expanding_time = Some(timer);
                    self.expanding_radius = Some(10.0);
                }
            }
            Event::AnimFrame(f) if self.expanding_time.is_some() => {
                let fraction_passed = (*f as f64) / 1.0e6 / 400.0;
                let to_expand_radii = (LABEL_INSETS.x_value() + self.label_size.width).max(100.0);
    ...
    fn paint(&mut self, ctx: &mut PaintCtx, data: &T, env: &Env) {
        ...
        let bg_color = if ctx.is_hot() { 
            env.get(crate::color::PRIMARY) 
        } else {
            env.get(crate::color::PRIMARY_VARIANT)
        };
        ctx.fill(rounded_rect, &bg_color);
        ctx.clip(rounded_rect);

        let circle_color = env.get(crate::color::ON_PRIMARY).with_alpha(0.2);
        // circles
        if let Some(r) = self.expanding_radius {
            let circle = Circle::new(self.click_pos.clone(), r);
